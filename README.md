...
This repo is pinned for Streamlit Cloud compatibility:

- `runtime.txt` uses **Python 3.10**
- `requirements.txt` pins TensorFlow CPU and supporting packages

If deployment fails during dependency installation:

1. Confirm your app points to this branch/commit with updated `runtime.txt` and `requirements.txt`.
2. Reboot the app from Streamlit Cloud settings (clears cached environment).
3. Ensure model files are available at your configured path (default `models/binary/best_model.keras`).

Typical reason for failure is Python/TensorFlow wheel mismatch on cloud runtimes.
