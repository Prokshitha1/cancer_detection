...
except Exception:  # pragma: no cover - runtime fallback for deployment env
    tf = None
...
def load_model(model_path: str) -> Any:
    if tf is None:
        raise RuntimeError(
            "TensorFlow is not available. Verify requirements.txt and Streamlit runtime Python version."
        )
...
if tf is None:
    st.error(
        "TensorFlow could not be imported in this deployment environment. "
        "Use runtime.txt with Python 3.10 and install pinned dependencies from requirements.txt."
    )
    st.stop()
